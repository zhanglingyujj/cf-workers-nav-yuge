...
test('服务器无值时回退 localStorage 镜像', () => {
    ...
    assert.deepEqual(s, { backgroundImage: 'https://example.com/a.jpg', backgroundOpacity: 35, backgroundBlur: 4 });
});

test('localStorage 也无值时用默认值', () => {
    assert.deepEqual(resolveSettings(null, makeStorage()), {
        backgroundImage: '', backgroundOpacity: 20, backgroundBlur: 0,
    });
});
...
test('服务器有值时取服务器值并补全缺省字段', () => {
    ...
    assert.deepEqual(s, { backgroundImage: 'https://example.com/b.jpg', backgroundOpacity: 20, backgroundBlur: 0 });
});
