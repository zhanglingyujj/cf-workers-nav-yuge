...
let currentEngine = localStorage.getItem('searchEngine') || 'site';
const engineList = ['site', 'baidu', 'bing', 'google'];

export function getEngineList() {
    return engineList.map(key => ({ key, label: searchEngineLabels[key] }));
}

export function setEngineByIndex(index) {
    selectEngine(engineList[index]);
}
