...
export async function handleGetSettings(request, env) {
    const corsHeaders = getCorsHeaders(env, request);
    const data = await env.CARD_ORDER.get(`settings_${DEFAULT_USER}`);
    return new Response(data || '{}', { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } });
}
...
export async function handleSaveSettings(request, env) {
    ...
    try {
        const body = await request.json();
        await env.CARD_ORDER.put(`settings_${DEFAULT_USER}`, JSON.stringify(body));
        return new Response(JSON.stringify({ success: true }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json'} });
    } catch (e) {
        return new Response(JSON.stringify({ error: 'Bad Request' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json'} });
    }
